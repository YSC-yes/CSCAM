...
def default_train(train_loader,model,optimizer,writer,iter_counter):
    ...
    # writer.add_scalar('G1',model.gamma_1.item(),iter_counter)
    # writer.add_scalar('G2',model.gamma_2.item(),iter_counter)
    # writer.add_scalar('G3',model.gamma_3.item(),iter_counter)
    # writer.add_scalar('G4',model.gamma_4.item(),iter_counter)
    avg_frn_loss = 0
    avg_aux_loss = 0
    avg_loss = 0
    avg_acc = 0
    ...
    avg_acc = avg_acc/(i+1)
    #print("循环结束的acc",avg_acc)
    avg_loss = avg_loss/(i+1)
    #print("avg_loss", avg_loss)
    avg_aux_loss = avg_aux_loss/(i+1)
    #print("avg_aux_loss", avg_aux_loss)
    avg_frn_loss = avg_frn_loss/(i+1)
    #print("avg_frn_loss", avg_frn_loss)
